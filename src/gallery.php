...
<script>
    var DEBUG = (function(){
        try{ return (parseInt(new URLSearchParams(location.search).get('debug')||'0',10) || 0) > 0; }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function openLightbox(index) {
        ...
        // Missing IPTC highlighting (debug only)
        if (DEBUG) {
            ...
            // Ensure no stale "missing" styling remains from previous opens
            [headlineWrapper, captionWrapper, keywordsWrapper, bylineWrapper, dateWrapper, specialWrapper]
              .forEach(function(w){ if(w) w.classList.remove('missing'); });
        }
